...
Se varió la resistencia de forma tal que, en el osciloscopio, se observara una curva con una pendiente pronunciada y sin sobrepico visible, intentando acercarnos lo mas posible al valor de la resistencia en el que $\omega_0 = \alpha_0$. No obstante, esta observación no implica que se haya alcanzado el valor real que minimiza el tiempo de estabilización.
...
A su vez, en el gráfico \textbf{REFERENCIAR EL GRAFICO SUB} se aprecia un período de oscilación mayor en el circuito de mayor capacitancia. Esta observación resulta coherente con las ecuaciones \textbf{REFEERENCIAR ECUACIONES DE WD} pues la capacitancia es inversamente proporcional a $\omega_d$ que a su vez es inversamente proportcional al Período T (\textbf{REFERENCIAR ECUACION DEL PERIODO}). 


\subsection{Opcional: Inductor con resistencia en paralelo}
Al simular el mismo circuito modelando las pérdidas del inductor mediante una resistencia en paralelo, se observó que, cuanto mayor era el valor de la resistencia en paralelo al inducto ( $R_P$ ), más similar resultaba la respuesta a la de un circuito RLC ideal. Esto se debe a que, al aumentar $R_P$, la corriente que circula por la resistencia disminuye y, en consecuencia, la potencia disipada en dicha rama es menor.

En cambio, al tomar valores más pequeños de $R_P$, una mayor fracción de la corriente circula por la rama resistiva en paralelo, lo que incrementa las pérdidas del sistema y atenúa el efecto inductivo ideal. Como consecuencia, la respuesta transitoria presenta un mayor amortiguamiento y una menor oscilación, semejandose mas a un circuito RC ideal.
...
\caption{Grafico en el tiempo de la diferencia de potencial en el capacitor ( V(nout))con $R_L$ en parlelo}
\label{fig:sim_g_RP} 
\end{figure}
...
\caption{Grafico en el tiempo de la diferencia de potencial en el capacitor ( V(nout)) con $R_L$ en serie}
\label{fig:sim_g_RS} 
\end{figure}

Al comparar la simulación utilizando un modelo de pérdidas en serie con aquella que emplea una resistencia en paralelo, se observa que en ambos casos la respuesta mantiene un comportamiento subamortiguado, con presencia de sobrepico y oscilaciones decrecientes. Sin embargo se pudieron encontrar algunas discrepancias entre ambos modelos, aunque no sean simplemente perceptibles a simple vista en los graficos.

Inicialmnete, se observo un leve aumento en la amplitud del sobrepico, siendo $V_C$ del sobrepico 7,71 V para el modelo con $R_P$ y 7,57 V para el modelo con resistencia serie. Este valor resulto más próximo al medido experimentalmente que fue de 7,78 V.
